...
namespace TMProX
{
    ...
    public partial class TextMeshProUGUI
    {
        ...
        protected override void OnEnable()
        {
            ...
            // Register text object for public updates
        ...
        public override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            tag_NoParsing = false;
            m_FontStylepublic = m_fontStyle;
            m_fontStyleStack.Clear();

            m_FontWeightpublic = (m_FontStylepublic & FontStyles.Bold) == FontStyles.Bold ? FontWeight.Bold : m_fontWeight;
            m_FontWeightStack.SetDefault(m_FontWeightpublic);
            ...
            if (m_textInfo == null)
                m_textInfo = new TMP_TextInfo(m_publicTextProcessingArraySize);
            else if (m_textInfo.characterInfo.Length < m_publicTextProcessingArraySize)
                TMP_TextInfo.Resize(ref m_textInfo.characterInfo, m_publicTextProcessingArraySize, false);
            ...
            for (int i = 0; i < unicodeChars.Length && unicodeChars[i].unicode != 0; i++)
            {
                ...
                if (m_isRichText && unicode == 60) // if Char '<'
                {
                    ...
                    if (ValidateHtmlTag(unicodeChars, i + 1, out endTagIndex))
                    {
                        ...
                        if ((m_FontStylepublic & FontStyles.Bold) == FontStyles.Bold)
                            m_isUsingBold = true;
                ...
                {
                    if ((m_FontStylepublic & FontStyles.UpperCase) == FontStyles.UpperCase)
                    {
                    ...
                    else if ((m_FontStylepublic & FontStyles.LowerCase) == FontStyles.LowerCase)
                    {
                        // If this character is uppercase, switch to lowercase.
                        if (char.IsUpper((char)unicode))
                            unicode = char.ToLower((char)unicode);
                    }
                    else if ((m_FontStylepublic & FontStyles.SmallCaps) == FontStyles.SmallCaps)
                    {
                ...
                #region LOOKUP GLYPH
                TMP_TextElement character = GetTextElement((uint)unicode, m_currentFontAsset, m_FontStylepublic, m_FontWeightpublic, out isUsingAlternativeTypeface);
                ...
                if (character == null)
                {
                    ...
                    character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, m_currentFontAsset, true, m_FontStylepublic, m_FontWeightpublic, out isUsingAlternativeTypeface);

                    if (character == null)
                    {
                        // Search for the missing glyph character in the TMP Settings Fallback list.
                        if (TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAssets((uint)unicode, m_currentFontAsset, TMP_Settings.fallbackFontAssets, true, m_FontStylepublic, m_FontWeightpublic, out isUsingAlternativeTypeface);
                    }
                    ...
                    if (character == null)
                    {
                        ...
                        if (TMP_Settings.defaultFontAsset != null)
                            character = TMP_FontAssetUtilities.GetCharacterFromFontAsset((uint)unicode, TMP_Settings.defaultFontAsset, true, m_FontStylepublic, m_FontWeightpublic, out isUsingAlternativeTypeface);
                    }
        ...
        protected override void OnCanvasHierarchyChanged()
        {
            ...
            // Special handling to stop publicUpdate calls when parent Canvas is disabled.
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                m_textInfo.characterInfo[m_characterCount].style = m_FontStylepublic;
                ...
                if (m_textElementType == TMP_TextElementType.Character && !isUsingAltTypeface && ((m_FontStylepublic & FontStyles.Italic) == FontStyles.Italic))
                {
